...
async def create_invoice(update, context):
    ...
    with SessionLocal() as session:
        with session.begin():
            ...
            if finacial_report.owner.wallet:
...
async def manual_check_zarinpal(update, context):
    ...
    with SessionLocal() as session:
        ...
        if payment_code == 100 and financial.payment_status not in ['paid', 'refund']:
            ...
            if error_code == 101:
                keyboard = [
                    [InlineKeyboardButton(text=await ft_instance.find_keyboard('login_to_payment_gateway'),url=f'https://payment.zarinpal.com/pg/StartPay/{financial.authority}')],
                    [InlineKeyboardButton(text=await ft_instance.find_keyboard('back_button'), callback_data="start_in_new_message")]
                ]
                reply_markup = InlineKeyboardMarkup(keyboard)
