        ...
        with (SessionLocal() as session):
            with session.begin():
                ...
                text = (
                    ...
                    f"\n\n{await ft_instance.find_text('vpn_service_id')} <code>{purchase.purchase_id}</code>"
                ...
                keyboard = [
                    [InlineKeyboardButton(await ft_instance.find_keyboard('vpn_change_service' if service_stauts_server == "active" else "vpn_upgrade_service"), callback_data=f'vpn_upgrade_service__{purchase.period}__{purchase.traffic}__{purchase_id}')],
        ...
        with SessionLocal() as session:
            ...
            keyboard = [
                ...
                [InlineKeyboardButton(await ft_instance.find_keyboard('vpn_change_service_ownership'), callback_data=f'vpn_change_service_ownership__{purchase_id}'),
                 InlineKeyboardButton(await ft_instance.find_keyboard('uptime_status'), url=f'http://uptime.freebyte.shop:3001/status/servers')],
