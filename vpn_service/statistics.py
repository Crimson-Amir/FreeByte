...
async def report_section(update, context):
    ...
    with SessionLocal() as session:
        ...
        text = (f'<b>{await ft_instance.find_text("usage_report")} {button_name}:</b>'
                f'\n\n• {await ft_instance.find_text("vpn_traffic_use_statistics")} {button_name}: {await vpn_utilities.format_traffic_from_megabyte(ft_instance, get_data[2])}'
                f'\n• {await ft_instance.find_text("avreage_usage_in")} {constituent_name}: {await vpn_utilities.format_traffic_from_megabyte(ft_instance, get_data[3])}')
        text += f'\n{detail_text}'
